# Performance Optimization PR (Final)
...
- **UseOfTimeManager Service**:
  ...
  - Implemented skill cost caching with proper null checks
...
- **Added null check in UseOfTimeManager**: Fixed TypeScript error by adding a null check before deleting from the skillCostCache map
- **Used Array.from() instead of spread operator**: Fixed TypeScript error by replacing `[...map.keys()]` with `Array.from(map.keys())` to avoid TS2802 error with iterators
- Note: Some TypeScript errors in E2E test files are unrelated to our optimization changes and should be addressed separately
...
The TypeScript fixes we've implemented are minimal and focused on resolving specific errors without changing the underlying optimization strategy.
