...
describe('NarrativeManager', () => {
  ...
  it('should trigger events when conditions are met', () => {
    ...
    expect(eventCallbackMock).toHaveBeenCalledWith(expect.objectContaining({
      id: 'test-event'
    }));
  });
  ...
  it('should update conspiracy tier based on arc progression', () => {
    ...
    // Add and discover multiple clues
    const cluesArc1 = Array(5).fill(null).map((_, i) => 
    ...
    const cluesArc2 = Array(5).fill(null).map((_, i) => 
      NarrativeFactory.createClue(
        `clue${i+6}`,
        `Clue ${i+6}`,
        `Description ${i+6}`,
