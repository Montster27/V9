...
import { NarrativeProgress, NarrativeType, TriggerType } from '../types/narrative';
...
export class NarrativeManager {
   ...
   */
  constructor(initialProgress?: NarrativeProgress, eventCallback?: (event: GameEvent) => void) {
  ...
  private checkArcProgression(): void {
    ...
    for (const arcId in arcs) {
      ...
      const discoveredArcClues = arc.clues.filter((clueId) => discoveredClues.includes(clueId));

      // Calculate new level based on discovered clues percentage
      const newLevel =
        Math.floor((discoveredArcClues.length / arc.clues.length) * arc.maxLevel) + 1;
  ...
  /**
   * Public method to get an event by ID
   * @param eventId Event ID to find
   * @returns The event object if found, undefined otherwise
   */
  public getEventById(eventId: string): GameEvent | undefined {
  ...
  public createInitialContent(): void {
    ...
    const initialEvents = [
      ...
      NarrativeFactory.createMysteryEvent(
        ...
        {
          'economics-professor':
            'Your economics professor seems nervous when you mention the late-night visitors',
